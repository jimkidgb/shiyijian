...
<table id="itemDatagrid" nowrap="false" class="easyui-datagrid" border="false" fit="true" rownumbers="true" singleSelect="true" fitColumns="true" autoRowHeight="true" toolbar="#focusTools" pagination="true" pageSize="10" url="product/getItemList">
	<thead>
		<tr>
...
function addItem(){
	$("<div>").dialog({href:"pages/wx/product/item.jsp",width:500, height:250, modal:true, title:"添加资源", buttons:[{text:"添加", handler:function () {
		$("#itemForm").attr("enctype","application/x-www-form-urlencoded");
		var d = $(this).closest(".window-body");
		$("#itemForm").form("submit", {url:"product/addItem", onSubmit:function () {
...
function editItem(){
	...
	if(row){
		$("<div>").dialog({href:"pages/wx/product/item.jsp",width:500, height:250, modal:true, title:"修改图文回复", buttons:[{text:"编辑", handler:function () {
			$("#itemForm").attr("enctype","application/x-www-form-urlencoded");
			var d = $(this).closest(".window-body");
			$("#itemForm").form("submit", {url:"product/editItem", onSubmit:function () {
				return $(this).form("validate");
			}, success:function (data) {
				if (data == "success") {
...
function removeItem(){
	...
	if (row) {
		$.messager.confirm("提示", "是否要删除当前选中的资源?", function (r) {
			if (r) {
				$.ajax({url:"product/removeItem", data:{id:row.id}, success:function (data) {
					if (data == "success") {
