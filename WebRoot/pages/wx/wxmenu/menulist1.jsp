...
<table id="wxMenuData1" singleSelect="true" rownumbers="true" fit="true" fitColumns="true" border="false" autoRowHeight="false" idField="id" treeField="name" animate="true" toolbar="#wxmenuTb1" data-options="lines:true,parentField:'pid'">
	<thead>
		<tr>
...
$(function () {
	$('#wxMenuData1').treegrid({url:"wxmenu/getMenuTree?account=ACCOUNT1"})
})
function addWxMenu1() {
...
function editWxMenu1() {
	var row = $("#wxMenuData1").treegrid("getSelected");
	if (row) {
...
function createMenu1(){
	$("<div>").dialog({href:"pages/wx/wxmenu/preview.jsp",width:320, height:480, modal:true,shadow:false,title:"自定义菜单预览", buttons:[{text:"更新到微信", handler:function () {
		...
		$.ajax({
			type:'post',
			url:'wxmenu/createMenu?account=ACCOUNT1',
			success: function(data){
				if(data == "notlist"){
		...
		$.ajax({
			type:'post',
			url:'wxmenu/getPreview?account=ACCOUNT1',
			success: function(data){
				if(data!=""){
					$("#htmlData").html(data);
