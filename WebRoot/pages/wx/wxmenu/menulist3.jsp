...
<table id="wxMenuData3" singleSelect="true" rownumbers="true" fit="true" fitColumns="true" border="false" autoRowHeight="false" idField="id" treeField="name" animate="true" toolbar="#wxmenuTb3" data-options="lines:true,parentField:'pid'">
	<thead>
		<tr>
...
$(function () {
	$('#wxMenuData3').treegrid({url:"wxmenu/getMenuTree?account=ACCOUNT3"})
})
function addWxMenu3() {
	$("<div>").dialog({href:"pages/wx/wxmenu/menu.jsp",width:560, height:280, modal:true,shadow:false,title:"添加自定义菜单", buttons:[{text:"添加", handler:function () {
		...
		$("#account").val("ACCOUNT3");
		$('#pid').combobox({url:'wxmenu/getMenuIsNull?account=ACCOUNT3'});
	}});
}
function editWxMenu3() {
	var row = $("#wxMenuData3").treegrid("getSelected");
	if (row) {
...
function createMenu3(){
	$("<div>").dialog({href:"pages/wx/wxmenu/preview.jsp",width:320, height:480, modal:true,shadow:false,title:"自定义菜单预览", buttons:[{text:"更新到微信", handler:function () {
		...
		$.ajax({
			type:'post',
			url:'wxmenu/createMenu?account=ACCOUNT3',
			success: function(data){
				if(data == "notlist"){
		...
		$.ajax({
			type:'post',
			url:'wxmenu/getPreview?account=ACCOUNT3',
			success: function(data){
				if(data!=""){
					$("#htmlData").html(data);
