...
public class GoodsDaoImpl extends DbUtils implements IMicroDao {

	@Override
	public boolean addItem(Item i) {
		String sql = "insert into t_wx_product_item values(null,?,?,?,?,now(),?,now())";
	...
	public boolean configMicro(Micro m) {
		if(m.getId() != null && !m.getId().equals("")){
			String sql = "update t_wx_product set title=?,picurl=?,modifier=?,modifydate=now() where id = ?";
			Object[] params = new Object[]{m.getTitle(),m.getPicurl(),m.getModifier(),m.getId()};
			return update(sql, params);
		}else{
			String sql = "insert into t_wx_product values (null,?,?,?,now(),?)";
	...
	public boolean editItem(Item i) {
		String sql = "update t_wx_product_item set title = ?,description=?,picurl=?,url=?,modifier=?,modifydate=now() where id = ?";
		Object[] params = new Object[] { i.getTitle(), i.getDescription(),
	...
	public String getItemCount() {
		String sql = "select count(*) from t_wx_product_item";
		return findBy(sql, 1).toString();
	}

	@Override
	public List<Item> getItemList(Integer page, Integer rows) {
		String sql = "select * from t_wx_product_item order by addtime desc limit ?,?";
		return find(Item.class, sql, new Object[] { (page - 1) * rows, rows });
	}

	@Override
	public boolean removeItem(String id) {
		String sql = "delete from t_wx_product_item where id = ?";
		return update(sql, id);
	}

	@Override
	public Micro getMicro(String menuid) {
		String sql = "select * from t_wx_product where menuid = ?";
